...
const Card: React.FC<ICardProps> = memo((props: ICardProps): JSX.Element => {
  const { className, children, max, padding = '8', variant = 'normal', border = 'normal', ...otherProps } = props
  ...
  return (
    <div
      {...otherProps}
      className={classNames(cls.card, { [cls.max]: max }, [className, cls[variant], cls[paddingClass], cls[border]])}
    >
      {children}
    </div>
  )
